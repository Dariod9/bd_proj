...
namespace BD_Proj
{
    public partial class Proprietario : Form
    {
        ...
        private void GetPessoas()
        {
            ...
            String sql = "SELECT * FROM proj_proprietario join proj_pessoa on proj_proprietario.nif=proj_pessoa.nif";
            SqlCommand com = new SqlCommand(sql, data.connection());
            SqlDataReader reader;
            reader = com.ExecuteReader();
            while (reader.Read())
            {
                ...
                tmp.nif = reader.GetDecimal(0);
                tmp.fname = reader.GetString(1);
                tmp.lname = reader.GetString(2);
                tmp.telefone = reader.GetDecimal(3);
                tmp.id = reader.GetDecimal(4);
